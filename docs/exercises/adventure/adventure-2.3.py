...
def do_go(args):
    ...
    if direction not in ('north', 'south', 'east', 'west'):
        print(f"Sorry, I don't know how to go: {direction}.")
        return
    ...
    new_name = old_place.get(direction.lower())

    # print an error if there is nothing in that direction
    if not new_name:
        print(f"Sorry, you can't go {direction} from here.")
        return
    ...
    if not new_place:
        print(f"Woops! The information about {new_name} seems to be missing.")
        return
...
def main():
    ...
    while True:
        print(f"You are at: {PLAYER['place']}")
